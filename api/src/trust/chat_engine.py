...
def classify_mode(query: str, has_documents: bool) -> str:
    ...
    if has_documents and any(term in normalized for term in DOCUMENT_TERMS):
...
async def _load_reviewed_evidence(
    ...
    rows = await client.request(
        ...
        "reviewed_claims",
        params={
            "select": "id,claim_key,statement_en,statement_hi,risk_level,source_passage_id,applicability_version,applicability_rule,effective_from,effective_to,revalidate_by,jurisdiction,lifecycle,reviewer_roles",
    ...
    for row in rows:
        ...
        if outcome.outcome != Outcome.APPLICABLE:
            continue
        searchable = f"{row.get('claim_key', '')} {row.get('statement_en', '')}"
        overlap = terms & _query_terms(searchable)
        score = len(overlap) * 3
        for term in terms:
            if term in searchable.casefold():
                score += 1
        if score:
    ...
    versions = await client.request(
        "GET", "source_versions",
        params={"select": "id,source_document_id,effective_from,effective_to,last_checked_at,fetch_status,review_status", "id": f"in.({','.join(version_ids)})"},
    )
    ...
    for _, row, reasons in candidates:
        ...
        if not passage or not version or not document:
            continue
        if version.get("fetch_status") != "healthy" or version.get("review_status") != "approved" or not document.get("active"):
            continue
        ...
        citations.append(SourceCitation(
            ...
            snippet=" ".join(passage["passage_text"].split())[:1200], source_tier=document["source_tier"],
            effective_from=version.get("effective_from"), effective_to=version.get("effective_to"),
            last_checked_at=version.get("last_checked_at"), reviewer_roles=row.get("reviewer_roles") or [],
        ))
...
async def build_chat_response(
    ...
    if req.business_id:
        ...
        coverage = _coverage_for(business)
    ...
    if mode == "reviewed_compliance":
        ...
        if not claims:
            return ChatResponse(
                answer=(
                    ...
                    "No legal or tax requirement has been inferred from model memory. Complete the missing business facts, inspect the coverage limits, or use the professional briefing below."
                ),
                answer_mode="professional_escalation", evidence_status="cannot_verify", language=req.language,
        ...
        return ChatResponse(
            ...
            language=req.language, claims=claims, citations=citations, coverage=coverage, effective_date=as_of,
